...
func main() {
	...
	window, err := glfw.CreateWindow(640, 480, "Overdrive", nil, nil)
	if err != nil {
		panic(err)
	}
    ...
    suzanne := mesh.Cube(geometry.ZeroVector(), geometry.ZeroVector(), geometry.NewVector(1, 1, 1))
	...
	for !window.ShouldClose() {
		...
		// camera.Position.AddAssign(geometry.NewVector(float64(speed)*float64(0), 0, float64(speed)*float64(-0)))
		// camera.Direction.AddAssign(geometry.NewVector(0, 0.01*float64(0), 0))
